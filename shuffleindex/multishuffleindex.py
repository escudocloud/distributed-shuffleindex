...
#from layers.multishufflelayer import *
#from layers.data.memorylayer import *
#from layers.statslayer import *
...
#
#N = 10                      # number of accesses to the datalayer in below tests
...
#levels = 3
#numdata = S ** (2 * levels)
#data = (printable * (1 + numdata // len(printable)))[:numdata]
#assert len(data) == numdata
#data = dict(enumerate(data))                          # create data
#statslayer = [_create_datalayer() for _ in xrange(S)]
#datalayer = MultiShuffleLayer(statslayer)
#tree = MultiTree(datalayer, fanout=S**2, leafsize=S)
#tree.bulk_load(data)            # bulk load data into the tree
#datalayer.set_root_ids(tree._roots)            # set the roots
#
#assert datalayer.get(0) == printable[0]

#    def test_search_with_not_included_key(self):
#        with pytest.raises(KeyError):
#            self.datalayer.get(len(self.data))
#
#    def test_with_shuffle_index(self):
#        for i in xrange(N):
#            self.datalayer.get(gaussrange(len(self.data)))
#
#        return # return earlier for now
